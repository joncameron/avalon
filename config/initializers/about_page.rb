AboutPage.configure do |config|
  ...
  config.environment  = AboutPage::Environment.new('Ruby/Rails' => /^(RUBY|RAILS|RACK|GEM_|rvm)/)
  config.request = AboutPage::RequestEnvironment.new('HTTP Server' => /^(SERVER_|POW_)/)
