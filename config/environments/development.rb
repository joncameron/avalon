Avalon::Application.configure do
  ...
  config.autoload_paths += Bundler.load.current_dependencies.map do |dep|
    if dep.source.is_a?(Bundler::Source::Path) && dep.source.options.key?('path')
  ...
  config.log_tags = ['AVALON',
                     :remote_ip,
                     proc { Time.now.strftime('%Y.%m.%d %H:%M:%S.%L') }]
