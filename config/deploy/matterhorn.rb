...
namespace :deploy do
  ...
  task :update_code do
    run "#{sudo as: 'matterhorn'} -i -- sh -c 'cd #{deploy_to}; rm -rf current; wget --no-check-certificate #{repository}/#{branch}.zip -O #{git_application}.zip; unzip #{git_application}.zip; mv #{git_application}-#{branch.tr('/', '-')} current; rm -f *.zip'"
  end
