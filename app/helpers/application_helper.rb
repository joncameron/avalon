...
module ApplicationHelper
  ...
  def image_for(document)
    ...
    video_count = begin
                    document['avalon_resource_type_tesim'].count { |m| m.start_with?('moving image') }
                  rescue
                    0
                  end
    audio_count = begin
                    document['avalon_resource_type_tesim'].count { |m| m.start_with?('sound recording') }
                  rescue
                    0
                  end
  ...
  def search_result_label(item)
    label = if item['title_tesi'].present?
              truncate(item['title_tesi'], length: 100)
            else
              item.id
            end
  ...
  def build_solr_request_from_response
    qs = @response['responseHeader']['params'].reject { |k, _v| k == 'wt' }.collect do |k, v|
