...
module MediaObjectsHelper
  ...
  def current_step_for(status = nil)
    status = HYDRANT_STEPS.first if status.nil?
  ...
  def class_for_step(context, step)
    css_class = if context.workflow.completed?(step)
                  'nav-success'
                else 'nav-disabled'
      end
  ...
  def display_notes(mediaobject)
    ...
    sorted_note_types.each do |note_type|
      ...
      notes.each_with_index do |note, i|
        note_string += "<p class='item_note_header'>#{note_types[note_type]}</p>" if i == 0 && note_type != 'general'
  ...
  def hide_sections?(sections)
    sections.blank? || (sections.length == 1 && sections.first.structuralMetadata.empty?)
  end
...
EOF
  ...
  def parse_section(section, _node, index)
    sectionnode = section.structuralMetadata.xpath('//Item')
    if sectionnode.children.present?
      tracknumber = 0
      contents = ''
    ...
    [contents, tracknumber]
  end

  def parse_node(section, node, tracknumber)
    if node.name.casecmp('DIV').zero?
      contents = ''
    ...
    elsif %w(SPAN ITEM).include? node.name.upcase
      tracknumber += 1
