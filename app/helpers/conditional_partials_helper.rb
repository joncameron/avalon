...
module ConditionalPartialsHelper
  def render_conditional_partials(partials_list_name, options = {})
    content = []
  ...
  def evaluate_if_unless_configuration(displayable_config, *args)
    return displayable_config if displayable_config === true || displayable_config === false

    if_value = !displayable_config.key?(:if) ||
               displayable_config[:if].nil? ||
               evaluate_configuration_conditional(displayable_config[:if], displayable_config, *args)

    unless_value = !displayable_config.key?(:unless) ||
