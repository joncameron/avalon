    ...
    cache_class = Class.new(ActiveFedora::SimpleDatastream) do
      ...
      def self.defined_attributes(ds)
        ...
        if @defined_attributes[ds].nil?
          ...
          owner_class.ds_specs[ds][:block].call(fc)
      ...
      def self.type(field)
        field_def = owner_class.defined_attributes[field.to_s]
        raise FieldError, "Unknown field `#{field}` for #{owner_class.name}" if field_def.nil?
        defined_attributes(field_def.dsid)[field.to_sym]
      end

      def primary_solr_name(field)
        ActiveFedora::SolrService.solr_name(field, type: type(field))
      end
