...
class StreamToken < ActiveRecord::Base
  ...
  def self.find_or_create_session_token(session, target)
    ...
    result = find_or_create_by_token_and_target(hash_token.to_s, target)
    result.renew!
  ...
  def self.logout!(session)
    session[:hash_tokens].each do |sha|
      find_all_by_token(sha).each &:delete
    end unless session[:hash_tokens].nil?
  end

  def self.purge_expired!
    where('expires <= :now', now: Time.now).find_each &:delete
  end
  ...
  def self.validate_token(value)
    ...
    if token.present? && token.expires > Time.now
      token.renew!
  ...
  def renew!
    update_attribute :expires, (Time.now + Avalon::Configuration.lookup('streaming.stream_token_ttl').minutes)
  end
end
