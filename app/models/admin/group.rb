...
module Admin
  class Group
    ...
    def name=(value)
      attribute_will_change!('name') if name != value && !@ignoring_changes
      @previous_name = name
      @name = value
    end
    ...
    def users=(value)
      attribute_will_change!('users') if users != value && !@ignoring_changes
      @users = value
    end
    ...
    def save
      if new_record? && valid? && !self.class.exists?(name)
        ...
        RoleControls.assign_users(users, name) if users_changed?
