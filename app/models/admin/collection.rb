...
class Admin::Collection < ActiveFedora::Base
  ...
  around_save :reindex_members, if: proc { |c| c.name_changed? || c.unit_changed? }
  ...
  def add_depositor(user)
    ...
    if self.edit_users.include? user
      raise ArgumentError, 'UserIsEditor'
    else
  ...
  class << self
    def reindex_media_objects(pid)
      collection = find pid
      collection.media_objects.each(&:update_index)
    end
  ...
  def to_solr(solr_doc = {}, *_args)
    ...
    solr_doc[Solrizer.default_field_mapper.solr_name('name', :facetable, type: :string)] = name
    solr_doc[Solrizer.default_field_mapper.solr_name('dropbox_directory_name', :facetable, type: :string)] = dropbox_directory_name
    solr_doc
  end
  ...
  def as_json(_options = {})
    {
      ...
      object_count: {
        ...
        unpublished: media_objects.reject(&:published?).count
      },
      roles: {
        managers: managers,
        editors: editors,
