...
module RightsMetadataExceptions
  def self.apply_to(mod)
    mod.class_eval do
      ...
      %w(edit read discover).each do |action|
        %w(groups users).each do |type|
      ...
      def preserved(type, permission)
        ...
        when :edit
          g = {}
        when :read
          Hash[quick_search_by_type(type).select { |_k, v| v == 'edit' }]
        when :discover
          Hash[quick_search_by_type(type).select { |_k, v| v == 'discover' }]
        end
      end
    end
  end
