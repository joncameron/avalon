...
class ExternalGroup
  def self.ldap_lookup(groupname, _opts = {})
    return [] unless defined? Avalon::GROUP_LDAP
    filter = Net::LDAP::Filter.eq('cn', groupname.to_s) & Net::LDAP::Filter.eq('objectclass', 'group')
  ...
  def self.ldap_autocomplete(query)
    ...
    ldap_lookup("#{query}*", size: 10)
  end

  def self.autocomplete(query)
    Course.autocomplete(query) + ldap_autocomplete(query)
  end
end
