  ...
  def populate_from_catalog!(bib_id, bib_id_label = nil)
    if bib_id.present?
      ...
      if new_record.present?
        old_resource_type = resource_type.dup
        old_media_type = media_type.dup
        old_other_identifier = other_identifier.type.zip(other_identifier)
        ...
        [:genre, :topical_subject, :geographic_subject, :temporal_subject,
         ...
         :title_subject].each do |field|
          send("#{field}=".to_sym, send(field).uniq)
        end
        old_media_type.each do |val|
          add_child_node(ng_xml.root, :media_type, val)
        end
        send('resource_type=', old_resource_type)
        languages = language.collect &:strip
        self.language = nil
        languages.each { |lang| add_language(lang) }
        new_other_identifier = other_identifier.type.zip(other_identifier)
        self.other_identifier = nil
        (old_other_identifier | new_other_identifier).each do |id_pair|
          add_other_identifier(id_pair[1], id_pair[0])
        end
      end
      add_other_identifier(bib_id, bib_id_label) unless other_identifier.type.zip(other_identifier).include?([bib_id_label, bib_id])
      self.bibliographic_id = nil
      add_bibliographic_id(bib_id, bib_id_label)
    end

    # Filter out notes that are not in the configured controlled vocabulary
    notezip = note.zip note.type
    self.note = nil
    notezip.each { |n| add_child_node ng_xml.root, :note, n[0], n[1] if NOTE_TYPES.include? n[1] }
  end
end
