...
class User < ActiveRecord::Base
  ...
  def self.find_for_identity(access_token, _signed_in_resource = nil)
  ...
  def self.find_for_lti(auth_hash, _signed_in_resource = nil)
    raise Avalon::MissingUserId if auth_hash.uid.blank?
  ...
  def self.autocomplete(query)
    where('username LIKE :q OR email LIKE :q', q: "%#{query}%").collect do |user|
  ...
  def destroy
    Bookmark.where(user_id: id).destroy_all
    super
  end
end
