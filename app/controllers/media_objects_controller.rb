...
class MediaObjectsController < ApplicationController
  ...
  before_action :authenticate_user!, except: [:show, :set_session_quality]
  before_action :authenticate_api!, only: [:show], if: proc { |_c| request.format.json? }
  ...
  before_action :inject_workflow_steps, only: [:edit, :update], unless: proc { |_c| request.format.json? }
  ...
  is_editor_or_not_lti = proc { |ctx| is_editor(ctx) || !is_lti_session(ctx) }
  is_editor_or_lti = proc { |ctx| (Avalon::Authentication::Providers.any? { |p| p[:provider] == :lti } && is_editor(ctx)) || is_lti_session(ctx) }
  ...
  def update_mediaobject
    ...
    if populate_from_catalog && Avalon::BibRetriever.configured?
      begin
        ...
        unless @mediaobject.valid?
          # Fall back to MODS as sent if Bib Import fails
          @mediaobject.update_datastream(:descMetadata, params[:fields].slice(*@mediaobject.errors.keys)) if params.key?(:fields) && params[:fields].respond_to?(:has_key?)
        end
      end
    else
      @mediaobject.update_datastream(:descMetadata, params[:fields]) if params.key?(:fields) && params[:fields].respond_to?(:has_key?)
    end
    ...
    if !@mediaobject.save
      ...
      oldparts = @mediaobject.parts.collect(&:pid)
      params[:files].each do |file_spec|
        ...
        if master_file.update_derivatives(file_spec[:files], false)
          ...
          error_messages += ["Problem saving MasterFile for #{begin
                                                                file_spec[:file_location]
                                                              rescue
                                                                '<unknown>'
                                                              end}:"] + master_file.errors.full_messages
          @mediaobject.destroy
          break
        end
      end
    ...
    if error_messages.empty?
      ...
      logger.warn "update_mediaobject failed for #{begin
                                                     params[:fields][:title]
                                                   rescue
                                                     '<unknown>'
                                                   end}: #{error_messages}"
  ...
  def custom_edit
    ...
    if 'preview' == @active_step
      ...
      if !@masterFiles.empty? && @currentStream.blank?
  ...
  def show_progress
    ...
    result = Hash[
      @mediaobject.parts.collect do |mf|
        mf_status = {
  ...
  def load_player_context
    return if request.format.json? && !params.key?(:content)

    if params[:part]
      index = params[:part].to_i - 1
      if index < 0 || index > @mediaobject.section_pid.length
