...
class ApplicationController < ActionController::Base
  ...
  around_action :handle_api_request, if: proc { |_c| request.format.json? }
  ...
  protect_from_forgery unless: proc { |_c| request.headers['Avalon-Api-Key'].present? }
  ...
  def set_access_control_headers
    ...
    headers['X-Frame-Options'] = 'ALLOWALL' if can_embed?
  end
  ...
  def get_user_collections
    if can? :manage, Admin::Collection
      ...
      Admin::Collection.where(ActiveFedora::SolrService.solr_name('inheritable_edit_access_person', Hydra::Datastream::RightsMetadata.indexer).to_s => user_key).to_a
    end
  end
