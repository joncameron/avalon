class PlaylistItemsController < ApplicationController
  ...
  def create
    unless (can? :create, PlaylistItem) && (can? :edit, @playlist)
      render(json: { message: 'You are not authorized to perform this action.' }, status: 401) && return
    end
    ...
    unless annotation.valid?
      render(json: { message: annotation.errors.full_messages }, status: 400) && return
    end
    unless annotation.save
      render(json: { message: "Item was not created: #{annotation.errors.full_messages}" }, status: 500) && return
    end
    if PlaylistItem.create(playlist: @playlist, annotation: annotation)
      render(json: { message: "Add to playlist was successful. See it: #{view_context.link_to('here', playlist_url(@playlist))}" }, status: 201) && return
    end
  rescue StandardError => error
    render(json: { message: "Item was not created: #{error.message}" }, status: 500) && (return)
  end
  ...
  def update
    ...
    if annotation.save
      render(json: { message: 'Item was updated successfully.' }, status: 201) && return
    else
      render(json: { message: "Item was not updated: #{annotation.errors.full_messages.join(', ')}" }, status: 500) && return
    end
  rescue StandardError => error
    render(json: { message: "Item was not updated: #{error.message}" }, status: 500) && (return)
  end
