...
class CatalogController < ApplicationController
  ...
  before_action :save_sticky_settings
  ...
  configure_blacklight do |config|
    ...
    config.add_facet_field 'workflow_published_sim', label: 'Published', limit: 5, if: proc { |context, _config, _opts| Ability.new(context.current_user, context.user_session).can? :create, MediaObject }, group: 'workflow'
    config.add_facet_field 'created_by_sim', label: 'Created by', limit: 5, if: proc { |context, _config, _opts| Ability.new(context.current_user, context.user_session).can? :create, MediaObject }, group: 'workflow'
    config.add_facet_field 'read_access_virtual_group_ssim', label: 'External Group', limit: 5, if: proc { |context, _config, _opts| Ability.new(context.current_user, context.user_session).can? :create, MediaObject }, group: 'workflow', helper_method: :vgroup_display
    config.add_facet_field 'date_digitized_sim', label: 'Date Digitized', limit: 5, if: proc { |context, _config, _opts| Ability.new(context.current_user, context.user_session).can? :create, MediaObject }, group: 'workflow' # , partial: 'blacklight/hierarchy/facet_hierarchy'
    config.add_facet_field 'date_ingested_sim', label: 'Date Ingested', limit: 5, if: proc { |context, _config, _opts| Ability.new(context.current_user, context.user_session).can? :create, MediaObject }, group: 'workflow'
  ...
  def apply_sticky_settings(solr_parameters, _user_parameters)
