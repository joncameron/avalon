...
class Admin::CollectionsController < ApplicationController
  ...
  def create
    ...
    if @collection.persisted?
      ...
      logger.warn "Failed to create collection #{begin
                                                   @collection.name
                                                 rescue
                                                   '<unknown>'
                                                 end}: #{@collection.errors.full_messages}"
  ...
  def update
    ...
    %w(manager editor depositor).each do |title|
    ...
    if can?(:update_access_control, @collection)
      %w(group class user ipaddress).each do |title|
        ...
        next unless params["remove_#{title}"].present?
    ...
    respond_to do |format|
      ...
      format.json do
        if saved
          ...
          logger.warn "Failed to update collection #{begin
                                                       @collection.name
                                                     rescue
                                                       '<unknown>'
                                                     end}: #{@collection.errors.full_messages}"
