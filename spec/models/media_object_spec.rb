...
describe MediaObject do
      ...
      let! (:valid_dates) do
        {
          ...
          '2006/2008' => %w(2006 2007 2008),
          '2004-01-01/2005' => %w(2004 2005),
          '2005-02-01/2006-02' => %w(2005 2006),
          '2006-03-01/2007-02-08' => %w(2006 2007),
          '2007/2008-02-01' => %w(2007 2008),
          '2008-02/2009-02-01' => %w(2008 2009),
          '2009-01-04/2010-02-01' => %w(2009 2010),
          '1984?' => ['1984'],
          '1984~' => ['1984'],
          '1984?~' => ['1984'],
          '2004-06-11?' => ['2004'],
          ...
          '2001-21' => ['2001'],
          '[1667,1668,1670..1672]' => %w(1667 1668 1670 1671 1672),
          '{1667,1668,1670..1672}' => %w(1667 1668 1670 1671 1672),
          '159u' => %w(1590 1591 1592 1593 1594 1595 1596 1597 1598 1599),
          '159u-12' => [],
          '159u-12-25' => %w(1590 1591 1592 1593 1594 1595 1596 1597 1598 1599),
          '159x' => %w(1590 1591 1592 1593 1594 1595 1596 1597 1598 1599),
          '2011-(06-04)~' => ['2011'],
  ...
  describe 'Languages are handled correctly' do
    it 'should handle pairs of language codes and language names' do
      media_object.update_datastream(:descMetadata, language: %w(eng French spa uig))
      expect(media_object.descMetadata.language_code.to_a).to match_array(%w(eng fre spa uig))
      expect(media_object.descMetadata.language_text.to_a).to match_array(%w(English French Spanish Uighur))
    end
  end
  ...
  describe 'Update datastream directly' do
    it 'should reflect datastream changes on media object' do
      ...
      media_object.save
      media_object.reload
      expect(media_object.bibliographic_id).to eq(%w(local ABC123))
    end
  end
  ...
  describe 'permalink' do
    ...
    before(:each) do
      Permalink.on_generate { |_obj, _target| 'http://www.example.com/perma-url' }
    end
    ...
    context 'correct target' do
      it 'should link to the correct target' do
        media_object.save(validate: false)
        t = nil
        Permalink.on_generate do |_obj, target|
          t = target
