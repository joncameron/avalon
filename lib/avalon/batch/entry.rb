    ...
    class Entry
      ...
      def media_object
        @media_object ||= MediaObject.new(avalon_uploader: @manifest.package.user.user_key,
          ...
          if Avalon::BibRetriever.configured? && fields[:bibliographic_id].present?
            begin
      ...
      def valid?
        ...
        media_object.valid?
        media_object.errors.messages.each_pair do |field, errs|
      ...
      def self.offset_valid?(offset)
        tokens = offset.split(':')
        return false unless (1...4).cover? tokens.size
        seconds = tokens.pop
