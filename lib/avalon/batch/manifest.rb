...
#          end
          ...
          @field_names.each_with_index do |f1, i|
            next if f1.blank? || SKIP_FIELDS.include?(f1) || values[i].blank?
            if FILE_FIELDS.include?(f1)
              content << {} if f1 == :file
              content.last[f1] = f1 == :skip_transcoding ? true?(values[i]) : values[i]
            else
              fields[f1] << values[i]
            end
          end
          ...
          entries << Entry.new(fields.select { |f1| !FILE_FIELDS.include?(f1) }, content, opts, index, self)
        end
      end
    end
  end
