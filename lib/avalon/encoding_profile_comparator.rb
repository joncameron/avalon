  ...
  class EncodingProfileComparator
    ...
    def self.find_invalid_profiles
      ...
      Derivative.find_each({}, batch_size: 10) do |d|
        epc = begin
                EncodingProfileComparator.new(d)
              rescue
                nil
              end
        next if epc.nil?
        derivatives_to_reencode << d unless epc.valid_profile?
      end
    ...
    def initialize(derivative)
      raise ArgumentError, 'Can only compare video files' unless derivative.masterfile.is_video?
