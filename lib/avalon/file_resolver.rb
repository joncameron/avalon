...
module Avalon
  class FileResolver
    ...
    def path_to(file_location)
      ...
      if url.scheme.nil? || url.scheme == 'file'
        mount_map.each_pair do |path, mount|
          next unless file_location.start_with? path
    ...
    def mount_map
      fstypes = %w(nfs cifs smbfs)
      @mount_map ||= Hash[
        mounts.collect do |l|
          (loc, mount, type) = l.scan(/^(.+) on (.+?) (?:type |\()([[:alnum:]]+)/).flatten
          next unless fstypes.include?(type)
          scheme = 'file'
          case type
          when 'nfs'
          ...
          loc = "#{scheme}://#{loc}"
          [File.join(mount, ''), loc]
        end.compact.sort { |a, b| b[0].length <=> a[0].length }
      ].merge(overrides)
    end
  end
end
